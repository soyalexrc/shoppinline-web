...
const ListingTabsTopSell: React.FC<Props> = ({ colSiderbar, variant='default',lang}) => {
    ...
    return (
        <div className="mb-8 lg:mb-15">
            <div className="listing-tabs">
                <ListingTabsList lang={lang} className={` py-2.5 rounded  ltabs-heading`} data={categories} onNavClick={handleTabClick} activeTab={activeTab}/>
                {isPending ? (
                    <div className="flex justify-center items-center min-h-[400px] bg-white">
                        <div className="animate-spin rounded-full h-12 w-12 border-4 border-blue-500 border-t-transparent"></div>
                    </div>
                ) : (
                    <ListingTabsContainer lang={lang} data={products} isLoading={isLoading} error={error}  variant={variant} />
                )}
