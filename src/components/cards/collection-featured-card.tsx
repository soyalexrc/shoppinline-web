      ...
      <div className="absolute  p-4 lg:p-9 left-5 bottom-5 lg:left-10 lg:bottom-10 bg-white rounded-md max-w-xs sm:max-w-sm">
        ...
        {btnText && (
          <Link
            href={`/${lang}${btnUrl}`}
            className={cn(
              'border-2 border-fill-base h-[44px] mt-5 md:mt-8 text-sm font-bold text-fill-base inline-flex transition duration-300 rounded px-10 py-2 hover:bg-brand hover:text-white hover:border-brand ',
            )}
          >
            {t(btnText)}
          </Link>
